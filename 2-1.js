...
const cleanInput = input.map(value => {
    const splitValues = value.split(" ");
    return [splitValues[0], splitValues[1].replace(":", ''), splitValues[2]]
})

const checkPassWordValidity = (minMax, letter, password) => {
    const {min, max} = getMinMax(minMax);
    let matchingCharacters = password.match(new RegExp(letter, "g"));
    matchingCharacters = matchingCharacters ? matchingCharacters.length : 0;

    return min <= matchingCharacters && matchingCharacters <= max;
}

const getMinMax = (minMax) => {
    const splitValues = minMax.split("-");
    return {
        min: splitValues[0],
        max: splitValues[1]
    }
...
const main = (cleanInput) => {
    let count = 0;
    cleanInput.forEach((input, i) => {
        if(checkPassWordValidity(input[0], input[1], input[2])) {
            count++;
        }
    })
    return count;
